...
public class WebHook : WebhookEventProcessor
{
    ...
    {
        Plugin.Config.GithubActions.TryGetValue(WebhookEventType.Release, out var group);

        if (!Plugin.Config.GithubActions.TryGetValue(WebhookEventType.Release, out var groups)
            || groups == null
            || groups.Count == 0)
            return;
    ...
    {
        if (!Plugin.Config.GithubActions.TryGetValue(WebhookEventType.Star, out var groups)
            || groups == null
            || groups.Count == 0)
            return;
    ...
    {
        if (!Plugin.Config.GithubActions.TryGetValue(WebhookEventType.Push, out var groups)
            || groups == null
            || groups.Count == 0)
            return;
    ...
    {
        if (!Plugin.Config.GithubActions.TryGetValue(WebhookEventType.PullRequest, out var groups)
            || groups == null
            || groups.Count == 0)
            return;
